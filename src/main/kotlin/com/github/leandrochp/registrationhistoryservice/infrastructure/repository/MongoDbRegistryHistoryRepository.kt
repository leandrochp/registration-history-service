package com.github.leandrochp.registrationhistoryservice.infrastructure.repository
...
import com.github.leandrochp.registrationhistoryservice.infrastructure.repository.entity.AddressEntity
import com.github.leandrochp.registrationhistoryservice.infrastructure.repository.entity.RegistryHistoryEntity
import com.github.leandrochp.registrationhistoryservice.infrastructure.repository.extension.toModel
...
class MongoDbRegistryHistoryRepository(
    ...
    override fun findByDocument(document: String): RegistryHistory? =
        logger.debug("Searching registry history by document: [$document]").run {
            BasicDBObject(mutableMapOf("document" to document).toMap())
        }.let {
            collection.find(it).projection(Projections.excludeId()).firstOrNull()?.let { document ->
                val jsonResult = document.toJson()
                logger.debug("Has found the follow registry history with the document: [$document, $jsonResult]")
                objectMapper.readValue<RegistryHistoryEntity>(jsonResult).toModel()
            }
        }
}
